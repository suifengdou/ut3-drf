...
class DWOSubmitViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = DealerWorkOrder.objects.filter(creator=user.username, order_status=1).order_by("id")
        return queryset
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            ...
            if order_ids:
                handle_list = DealerWorkOrder.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
