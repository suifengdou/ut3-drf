...
class ExceptionIPO(models.Model):
    ORDER_STATUS = (
        (0, '已取消'),
        (1, '待处理'),
        (2, '已完成'),
    ...
    ori_order_id = models.ForeignKey(IntPurchaseOrder, on_delete=models.CASCADE, verbose_name='采购单', help_text='采购单')
    order_id = models.CharField(unique=True, null=True, blank=True, max_length=200, db_index=True, verbose_name='异常单号', help_text='异常单号')
    distributor = models.ForeignKey(IntDistributor, on_delete=models.CASCADE, verbose_name='经销商', help_text='经销商')
    order_category = models.SmallIntegerField(choices=CATEGORY_LIST, default=1, verbose_name='类型', help_text='类型')
    contract_id = models.CharField(null=True, blank=True, max_length=150, verbose_name='合同编号', help_text='合同编号')
    ...
    amount = models.FloatField(default=0, verbose_name='异常单总金额', help_text='异常单总金额')
    quantity = models.IntegerField(default=0, verbose_name="货品总数", help_text="货品总数")
    address = models.CharField(null=True, blank=True, max_length=250, verbose_name='地址', help_text='地址')
    memo = models.CharField(null=True, blank=True, max_length=250, verbose_name='备注', help_text='备注')
    ...
    mistake_tag = models.SmallIntegerField(choices=MISTAKE_LIST, default=0, verbose_name='错误标签', help_text='错误标签')
    process_tag = models.SmallIntegerField(choices=PROCESS_TAG, default=0, verbose_name='处理标签', help_text='处理标签')
    exception_type = models.IntegerField(choices=EXCEPTION_LIST, default=0, verbose_name='异常类型', help_text='异常类型')
    created_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间', help_text='创建时间')
    updated_time = models.DateTimeField(auto_now=True, verbose_name='更新时间', help_text='更新时间')
    is_delete = models.BooleanField(default=False, verbose_name='删除标记', help_text='删除标记')
    creator = models.CharField(null=True, blank=True, max_length=150, verbose_name='创建者', help_text='创建者')
    ...
    class Meta:
        verbose_name = 'INT-国际采购异常单'
        verbose_name_plural = verbose_name
        db_table = 'int_purchase_order_exception'
        permissions = (
            ('view_user_exceptionipo',  'Can view user INT-国际采购异常单'),
            ('view_handler_exceptionipo', 'Can view handler INT-国际采购异常单'),
        )
...
class EIPOGoods(models.Model):
    ...
    eipo = models.ForeignKey(ExceptionIPO, on_delete=models.CASCADE, verbose_name='采购异常单', help_text='币种')
    ...
    order_status = models.SmallIntegerField(choices=ORDER_STATUS, default=1, verbose_name='货品状态', help_text='货品状态')
    ...
    class Meta:
        verbose_name = 'INT-国际采购异常单-货品明细'
        verbose_name_plural = verbose_name
        unique_together = (('eipo', 'goods_name'))
        db_table = 'int_purchase_order_exception_goods'
        permissions = (
            ('view_user_eipogoods',  'Can view user INT-国际采购异常单-货品明细'),
            ('view_handler_eipogoods', 'Can view handler INT-国际采购异常单-货品明细'),
        )
