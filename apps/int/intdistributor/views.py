...
from .filters import IntDistributorFilter, ContactModeFilter,ContactsFilter
from .models import IntDistributor, ContactMode, Contacts
...
class ContactModeViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品
    list:
    ...
    partial_update:
        更新部分货品字段
    """
    queryset = ContactMode.objects.all().order_by("id")
    serializer_class = ContactModeSerializer
    filter_class = ContactModeFilter
    ...
    def save_resources(request, resource):
        ...
        for row in resource:
            _q_goods = ContactMode.objects.filter(goods_id=row["goods_id"])
            if _q_goods.exists():
                order = _q_goods[0]
            else:
                order = ContactMode()
            ...
            for field in order_fields:
                if field == "goods_attribute":
                    ...
                    _q_category = ContactMode.objects.filter(name=row["category"])
...
class ContactsMyselfViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品
    list:
    ...
    def get_queryset(self):
        ...
        queryset = Contacts.objects.filter(creator=user.username).order_by("id")
        return queryset
...
class IntDistributorMyselfViewset(viewsets.ModelViewSet):
    ...
    partial_update:
        更新部分货品字段
    """
    serializer_class = IntDistributorSerializer
    ...
    def handle_upload_file(self, request, _file):
        ...
        if '.' in _file.name and _file.name.rsplit('.')[-1] in ALLOWED_EXTENSIONS:
            ...
            # 验证一下必要的核心字段是否存在
            _ret_verify_field = IntDistributor.verify_mandatory(columns_key)
            if _ret_verify_field is not None:
                return _ret_verify_field
...
class IntDistributorViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品
    list:
