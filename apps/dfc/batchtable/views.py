        ...
        if n:
            for obj in check_list:
                ...
                for words in seg_list:
                    if not obj.city:
                        ...
                        special_city = ['仙桃市', '天门市', '神农架林区', '潜江市', '济源市', '五家渠市', '图木舒克市', '铁门关市', '石河子市', '阿拉尔市',
                                        ...
                                        '陵水黎族自治县', '临高县', '乐东黎族自治县', '东方市', '定安县', '儋州市', '澄迈县', '昌江黎族自治县', '保亭黎族苗族自治县',
                                        '白沙黎族自治县', '中山市', '东莞市']
                        if obj.city.name in special_city:
                            break
                        _q_district = District.objects.filter(name__contains=words)
                        if not _q_district:
                            continue
                        else:
                            obj.district = _q_district[0]
                            break

                if obj.city:
                    obj.province = obj.city.province
                _prefix = "MO"
                ...
                for goods_detial in all_goods_detials:
                    _q_export_mo = OriginData.objects.filter(mobile=obj.mobile, goods_id=goods_detial.goods_id)
    ...
    def handle_upload_file(self, request, _file):
        ...
        if '.' in _file.name and _file.name.rsplit('.')[-1] in ALLOWED_EXTENSIONS:
            ...
            filter_fields = ["店铺", "店铺名称", "订单号", "订单编号", "销售订单号", "下单帐号", "买家会员名", "ID",
                             "客户网名", "用户昵称", "网名", "客户姓名", "收货人姓名", "姓名", "顾客姓名", "收件人",
    ...
    def check(self, request, *args, **kwargs):
        ...
        if n:
            for obj in check_list:
                ...
                all_goods_detials = obj.mogoods_set.all()
                for goods_detial in all_goods_detials:
                    _q_export_mo = BatchTable.objects.filter(mobile=obj.mobile, goods_id=goods_detial.goods_id)
                    if _q_export_mo.exists():
                        if obj.process_tag != 3:
                            delta_date = (obj.create_time - _q_export_mo[0].create_time).days
                            if int(delta_date) < 14:
